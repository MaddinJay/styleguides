...
|------------|-----------------------|-------------|
| Primitive Obsession/Type Codes   | Some units are represented by variables with a primitive types for characters or numbers.Unfortunately in most cases it is not a proper way to represent some data and a higher abstraction level like objects is highly recommended. In other case suddenly we have to cope with several comparisons of strings and numbers, which leads to **nested conditional statements** and **repeated switches** based on **type codes**. On other hand we get problem, when some additional functionality like conversion  has to be implemented, because the code can be everywhere in the software. So primitive obsession breaks also **DRY-Principle** and make it very hard to maintain the code. However on the first sight it can look like a **very high generic solution**, but in the reality it is a **strong dependency to procedural paradigm** of programming. Also in the **parameter lists** primitive obsession can have a negative impact on the method body.| Replace Primitive with Object/Introduce Null Object(Introduce Special Case), Introduce Object Parameter, Replace Type Code with Subclasses. |
| Repeated Switches/Nested Conditionals/Conditional Complexity | Conditional Statements are the most common used elements of every programming language. 
They are also the most misused control structures. Nested conditionals have too **many logical expressions** on different **levels of control flow**. It leads often to **long functions**, which break **SRP-Principle**. Any new condition generates new **nested part** or **increases the logical expressions**. In many cases some conditionals are after a time just a **dead code** or there are empty conditionals without meaning like empty methods in inheritance, that breaks Liskov Substitution. Very often the starting point to nested conditionals are **long flags parameters**.| Decompose Conditional/Introduce Guard Statements/Reverse Conditionals/Remove Dead Code Replace Primitive with Object/Introduce Null Object(Introduce Special Case), Remove flag argument, 
Replace Type Code with Subclasses, extract function/extract class, move function.|
| Long Functions | Too many logic in the main flow leads to long functions, which breaks automatically **SRP-Principles**. Long functions are hard to read and can destroy **cohesion** of the class. Starting points to this smell code are in most cases other smells like **nested conditionals**, **long parameter list**, **feature envy** or **data clumps**. | Extract Function/Introduce Parameter Object/Decompose Conditionals/Split Loop/ Replace Conditional with Polymorphism.  |
