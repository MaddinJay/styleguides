...
|------------|-----------------------|-------------|
| Primitive Obsession/Type Codes   | Some units are represented by variables with a primitive types for characters or numbers. 
Unfortunately in most cases it is not a proper way to represent some data and a higher abstraction level like objects is highly recommended. In other case suddenly we have to cope with several comparisons of strings and numbers, which leads to **nested conditional statements** and **repeated switches** based on **type codes**. On other hand we get problem, when some additional functionality like conversion  has to be implemented, because the code can be everywhere in the software. So primitive obsession breaks also **DRY-Principle** and make it very hard to maintain the code. However on the first sight it can look like a **very high generic solution**, but in the reality it is a **strong dependency to procedural paradigm** of programming. 
Also in the **parameter lists** primitive obsession can have a negative impact on the method body.| Replace Primitive with Object/Introduce Null Object(Introduce Special Case), Introduce Object Parameter, 
